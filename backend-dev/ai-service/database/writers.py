...
def insert_demand_forecasts_batch(conn, forecasts: List[Dict[str, Any]]) -> List[int]:
    """Insert multiple demand forecasts in a single transaction"""
    if not forecasts:
        return []
    ...
    with conn.cursor() as cur:
        for forecast in forecasts:
            cur.execute(query, forecast)
            ids.append(cur.fetchone()[0])
        conn.commit()

    logger.info(f"Batch inserted {len(ids)} demand forecasts")
    return ids


def insert_classifications_batch(conn, classifications: List[Dict[str, Any]]) -> List[int]:
    """Insert multiple classifications in a single transaction"""
    ...
    with conn.cursor() as cur:
        for cls in classifications:
            cur.execute(query, cls)
            ids.append(cur.fetchone()[0])
        conn.commit()

    logger.info(f"Batch inserted {len(ids)} classifications")
    return ids


def insert_cluster_results_batch(conn, clusters: List[Dict[str, Any]]) -> List[int]:
    """Insert multiple cluster results in a single transaction"""
    if not clusters:
        return []
    ...
    logger.info(f"Batch inserted {len(ids)} cluster results")
    return ids


def insert_supplier_scores_batch(conn, scores: List[Dict[str, Any]]) -> List[int]:
    """Insert multiple supplier scores in a single transaction"""
    if not scores:
        return []
    ...
    logger.info(f"Batch inserted {len(ids)} supplier scores")
    return ids
