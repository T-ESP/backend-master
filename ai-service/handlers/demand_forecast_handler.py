        ...
        try:
            ...
            for result in results:
                ...
                try:
                    ...
                    self.logger.warning(f"Failed to insert forecast for product {product_id}: {e}")
                    try:
                        conn.rollback()
                    except Exception:
                        pass
                ...
                if urgency in ["URGENT", "HIGH"]:
                    ...
                    try:
                        ...
                        self.logger.warning(f"Failed to insert notification for product {product_id}: {e}")
                        try:
                            conn.rollback()
                        except Exception:
                            pass

            self.logger.info(f"Demand forecast completed: {successful} successful, {inserted} inserted, {failed} failed")
